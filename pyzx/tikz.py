...
from typing import List, Dict, overload, Tuple, Union, Optional, Set
...
def tikz_to_graph(
    ...
    Args:
        ...
        ignore_invalid_phases: If True, use default phase (0 for Z/X spiders, 1 for H-boxes)
            when an invalid phase label is encountered instead of raising an error.
        ...
        remove_identity_nodes: If True, remove 'none' style nodes with exactly 2 neighbors,
    ...
    for c,l in enumerate(lines[2:]):
        ...
        try:
            ...
            if ignore_parse_errors:
                continue
            raise ValueError("Failed to parse node definition '%s': %s" % (l, str(ex)))
    ...
    for c2,l in enumerate(lines[c+4:]):
        ...
        try:
            ...
            if src_id not in index_dict or tgt_id not in index_dict:
                if ignore_parse_errors:
                    continue
                raise ValueError("Edge references non-existent node in definition %s" % l)

            e = index_dict[src_id], index_dict[tgt_id]
        except Exception as ex:
            if ignore_parse_errors:
                continue
            raise ValueError("Failed to parse edge definition '%s': %s" % (l, str(ex)))
    ...
    if remove_identity_nodes:
        for v in list(none_style_vertices):
            ...
            if len(neighbors) == 2:
                ...
                new_et = EdgeType.SIMPLE if et1 == et2 else EdgeType.HADAMARD
                g.remove_vertex(v)
